...
Airflow expects python files with Airflow dags present in the [dags_folder](https://airflow.apache.org/docs/apache-airflow/2.2.0/configurations-ref.html#dags-folder). By default this terraform template sets that as `/dags`. This can be changed via the `airflow_dags_folder` [variable](./variables.tf). The metaflow-tools repository also ships a [dag-copy.py](../../scripts/dag-copy.py) file that can help sync airflow dag files generated by Metaflow to the Airflow scheduler. Under the hood [dag-copy.py](../../scripts/dag-copy.py) uses the `kubectl cp` command to copy files from local to the Airflow scheduler's container. Example of how to use the file:
```
python dag-copy.py my-dag.py /dags/my-dag.py
```
### Common issues:
