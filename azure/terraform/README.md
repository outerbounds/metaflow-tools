...
The metaflow-tools repository also ships a [dag-copy.py](../../scripts/dag-copy.py) file that can help sync airflow dag files generated by Metaflow to the Airflow scheduler _deployed by this template_. Under the hood [dag-copy.py](../../scripts/dag-copy.py) uses the `kubectl cp` command to copy files from local to the Airflow scheduler's container. Example of how to use the file:
```
python dag-copy.py my-dag.py /opt/airflow/dags/my-dag.py
```
### Common issues:
