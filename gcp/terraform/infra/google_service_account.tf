# TODO rename this guy... this GSA will be used for workload identity AND somewhat general use.
...
resource google_project_iam_binding "metaflow_kubernetes_workload_identity_service_account_cloud_sql_client_role_binding" {
  provider = google-beta
  project = var.project
  role    = "roles/cloudsql.client"
  members = [
    "serviceAccount:${google_service_account.metaflow_kubernetes_workload_identity_service_account.email}"
  ]
  condition {
...
resource google_project_iam_binding "metaflow_kubernetes_workload_identity_service_account_kubernetes_user_role_binding" {
  provider = google-beta
  project = var.project
...
resource google_project_iam_binding "metaflow_kubernetes_workload_identity_service_account_storage_role_binding" {
  provider = google-beta
  project = var.project
